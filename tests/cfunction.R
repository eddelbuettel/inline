# Tests adapted from the examples in man/cfunction.Rd
library(inline)
...
res_1 <- cubefn(n, x)
stopifnot(identical(res_1, res_cube))
...
res_2 <- cubefn2(n, x)
stopifnot(identical(res_2, res_cube))
...
res_3 <- cubefn3(n, x)
stopifnot(identical(res_3, res_cube))

## Same example in C, using a user-defined name for the generated function
code4 <- "
      int i;
      for (i = 0; i < *n; i++)
        x[i] = x[i]*x[i]*x[i];
...
cubefn4 <- cfunction(signature(n = "integer", x = "numeric"), code4,
  language = "C", convention = ".C", name = "cube4")
res_4 <- cubefn4(n, x)
stopifnot(identical(res_4, res_cube))
...
stopifnot(all.equal(res_5$x, c(pi, exp(1)), tolerance = 1e-7))
...
stopifnot(
  identical(res_6_square, res_square),
  identical(res_6_quad, res_quad))
...
stopifnot(
  identical(res_7_square, res_square),
  identical(res_7_quad, res_quad))
